<?php
    ...
    require_once "models/ProfilePostUtility.php";
...
<script>
    ...
    function replyTogglers() {
        const hiddenReplyForm = document.querySelectorAll('.hidden-reply-form');
        const hiddenTextAreas = document.querySelectorAll('.hidden-text-area');
        const cancelBtn = document.querySelectorAll('.cancel-btn');
        const replyToggler = document.querySelectorAll('.reply-toggler');

        replyToggler.forEach(value => {
            value.addEventListener('click', e => {
                const parentContainer = e.currentTarget.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.children[1];
                replyToggler.forEach(value => {
                    value.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.children[1].classList.remove('enabled-reply-form');
                    value.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.children[1].classList.add('disabled-reply-form');
                });
                parentContainer.classList.add('enabled-reply-form');
                parentContainer.classList.remove('disabled-reply-form');
            });
        })

        hiddenReplyForm.forEach(value => {
            value.addEventListener('click', e => {
            });
        });

        cancelBtn.forEach(value => {
            value.addEventListener('click', e => {
                const parentContainer = e.currentTarget.parentNode.parentNode.parentNode.parentNode;
                parentContainer.classList.add('disabled-reply-form');
                hiddenTextAreas.forEach(value => {
                    value.value = '';
                });
            });
        });
        ...
        const commentReplyBox = document.querySelectorAll('.comment-textarea');

        commentReplyBox.forEach((value) => {
            value.addEventListener('focus', (e) => {
                e.currentTarget.nextElementSibling.style.display = 'inline-block';
                e.currentTarget.nextElementSibling.nextElementSibling.style.display = 'inline-block';
                
                e.currentTarget.nextElementSibling.addEventListener('click', (e) => {
    ...
    $(document).ready(function() {
        ...
        $('body').on('submit', '#new-post-form', function(event) {
            event.preventDefault();
            let postData = document.getElementById('wall-post').value;
            
            let params = "type=new-post&wall-post=" + postData;

            let xhr = new XMLHttpRequest();

            xhr.open('POST', 'controllers/ProfilePosts.php', true);
            xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');

            xhr.onload = function() {
                if(this.status == 200) {
                    document.querySelectorAll('.profile-posts')[0].innerHTML = this.responseText;
                    replyTogglers();
                    latestComment();
                }
            }
    ...
    function latestComment() {
        ...
        let commentFormId = commentForms[0].id;
        let id = commentFormId.replace( /^\D+/g, '');
        
        $('body').on('submit', `#comment-form-${id}`, function(event) {
                event.preventDefault();
                
                let type = 'post-reply';
                let replyParentName = document.getElementById(`reply-parent-name-${id}`).value;
                let replyParentId = document.getElementById(`reply-parent-id-${id}`).value;
                let comment = document.getElementById(`comment-${id}`).value;

                let params = "type=" + type + "&reply-parent-name=" + replyParentName + 
                "&reply-parent-id=" + replyParentId + "&comment=" + comment;

                let xhr = new XMLHttpRequest();

                xhr.open('POST', 'controllers/ProfilePostsReplies.php', true);
                xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    ...
    function commentFunction() {
        ...
        commentForms.forEach(value => {
            let commentFormId = value.id;
            let id = commentFormId.replace( /^\D+/g, '');
